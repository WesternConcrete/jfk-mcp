...
server.tool("jfk-text-search", "Perform a text search on JFK files using a query string and optional metadata filters. Should only use 1 word for the query, since it does an exact match. NARA release dates between 2017-2025", TextSearchInputSchema, async (input) => {
...
server.tool("jfk-vector-search", "Perform a vector search on JFK files using a query string and optional metadata filters (excluding comments). NARA release dates between 2017-2025", VectorSearchInputSchema, async (input) => {
    const { query, metadata, limit } = input;
    try {
...
server.tool("jfk-metadata-search", "Perform a metadata search on JFK files using detailed metadata filters. NARA release dates between 2017-2025", MetadataSearchInputSchema, async (input) => {
...
server.tool("jfk-get-page-text", "Retrieve text content for specific pages of a JFK document", PageInputSchema, async (input) => {
...
server.tool("jfk-get-page-png", "Retrieve PNG images for specific pages of a JFK document", PageInputSchema, async (input) => {
    const { page_ids } = input;
    try {
